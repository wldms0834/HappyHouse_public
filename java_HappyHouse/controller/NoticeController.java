...
public class NoticeController {
	...
	@ApiOperation(value = "모든 공지글의 정보를 반환한다.", response = List.class)
	@GetMapping
	...
	@GetMapping("{noticeNo}")
	public ResponseEntity<Notice> detailNotice(@PathVariable int noticeNo, Model model) {
		ResponseEntity<Notice> notice = new ResponseEntity<Notice>(noticeService.selectNoticeByNo(noticeNo), HttpStatus.OK);
		logger.debug("detailNotice board 호출");
		//model.addAttribute("detailNotice",qna.getBody().getQnaUserid());
		//logger.debug(qna.getBody().getQnaUserid());
		return notice;
	}
